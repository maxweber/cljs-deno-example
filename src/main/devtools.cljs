(ns devtools
  (:require
    ["https://deno.land/x/websocket@v0.1.4/mod.ts" :as ws]
...
(defn start [runtime]
  (let [ws-url
        ...
        socket
        (new ws/StandardWebSocketClient ws-url)
    ...
    (.on socket "message"
         (fn [data]
           ;; (prn (.-data data))
           (when @ws-active-ref
             (cljs-shared/remote-msg runtime (.-data data)))))
